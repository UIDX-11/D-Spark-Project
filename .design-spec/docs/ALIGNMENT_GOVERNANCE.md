...
| **对外 API** | **Arco Vue**（README + `packages/web-vue/components/<name>/`） | props / slots / events / emits、默认受控与非受控、校验触发时机等与 Vue 实现一致。 |
| **DOM 顺序与语义** | **默认 Arco Vue** | 若 Figma 明确要求可 Tab 等结构，允许与 Arco DOM **有文档化差异**（见组件 MD「与 Arco 差异」）。 |
...
- **必须对齐**：props / slots / events、默认行为、键盘与受控/校验时机（以 Vue 为准）。
- **默认对齐**：DOM **节点顺序**、可交互元素类型；仅在 Figma 强制要求时偏离，并在 MD 记录。
- **类名**：不要求与 `arco-*` 字符串一致；设计稿 demo 使用 **`ds-*`** 时，须在 MD 的 References 中写明对照的 Arco 源文件路径或 URL。
...
- **Arco 版本升级**：视觉不随 Arco patch 漂移；对齐记录以 **固定 Figma 节点 + 固定 Vue 参考版本** 为准（写在各组件 MD 的 References）。
...
- **MD**：规范、References（Figma node + Arco Vue）、token 表、与 Arco/Figma 差异、推断态说明。
...
**Figma 未覆盖的状态**：按 **Arco Vue 行为 + 设计系统推断** 实现，并在 MD 增加 **「推断」** 小节，注明依据（源码位置或 README 条款 + 所用 token）。
