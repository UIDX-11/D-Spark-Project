    ...
    for md in docs:
        ...
        for pattern, regex in zip(patterns, compiled):
            if regex.search(text):
                ...
                errors.append(f"{rel}: forbidden pattern '{pattern}'")
    return errors


def main() -> int:
    cfg = load_config()
    if cfg.get("behaviorSource") != "arco-vue":
        print("SKIP: behaviorSource is not arco-vue")
        return 0

    docs = iter_component_docs(cfg["docsScope"])
    errors = scan_forbidden_patterns(docs, cfg.get("forbiddenPatterns", []))
    if errors:
        print("FAIL: Vue truth-source guard detected forbidden references:")
        for err in errors:
            print(f"- {err}")
        return 1

    print(f"PASS: Vue truth-source guard checked {len(docs)} component docs.")
    return 0
